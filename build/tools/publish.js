import { z } from "zod";
import { createMediaContainer, waitForContainerReady, publishContainer, toolResult, toolError, } from "../client.js";
export function registerPublishTools(server) {
    server.registerTool("publish_photo", {
        title: "Publish Photo",
        description: "Publish a single photo to Instagram feed. Requires a public image URL. Optionally add a caption, location, and user tags.",
        inputSchema: {
            ...
            user_tags: z
                .string()
                .optional()
                .describe('JSON array of user tags: [{"username":"user1","x":0.5,"y":0.5}]. Coordinates are 0-1 range.'),
        },
        annotations: {
            readOnlyHint: false,
        ...
        try {
            ...
            return toolError(`Failed to publish photo: ${error.message}`);
        }
    });
    server.registerTool("publish_reel", {
        title: "Publish Reel",
        description: "Publish a Reel (short video) to Instagram. Requires a public video URL. Video processing may take time.",
        inputSchema: {
        ...
        try {
            ...
            return toolError(`Failed to publish reel: ${error.message}`);
        }
    });
    server.registerTool("publish_carousel", {
        title: "Publish Carousel",
        description: "Publish a carousel (album) post with 2-10 images/videos. Each item needs a public URL. Videos require processing time.",
        inputSchema: {
            items: z
                .string()
                .describe('JSON array of items: [{"type":"IMAGE","url":"https://..."},{"type":"VIDEO","url":"https://..."}]. Type: IMAGE or VIDEO.'),
            caption: z.string().optional().describe("Post caption (max 2200 characters)"),
            location_id: z.string().optional().describe("Facebook Page ID for location tag"),
        },
        annotations: {
        ...
        try {
            ...
            return toolError(`Failed to publish carousel: ${error.message}`);
        }
    });
    server.registerTool("publish_story", {
        title: "Publish Story",
        description: "Publish a story (image or video) to Instagram. Stories disappear after 24 hours.",
        inputSchema: {
            media_type: z
                .enum(["IMAGE", "VIDEO"])
        ...
        try {
            ...
            return toolError(`Failed to publish story: ${error.message}`);
        }
    });
}
