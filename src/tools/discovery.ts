...
export function registerDiscoveryTools(server: McpServer) {
  server.registerTool(
    "search_hashtag",
    {
      ...
      description:
        "Search for an Instagram hashtag and get its ID. Use the returned ID with get_hashtag_top_media. Limited to 30 unique hashtag searches per 7 days.",
      inputSchema: {
    ...
    async (args) => {
      try {
        const data = await apiRequest(
          ...
          "GET",
          undefined,
          { q: args.query, user_id: INSTAGRAM_ACCOUNT_ID },
        );
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to search hashtag: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    "get_hashtag_top_media",
    {
      ...
      description:
        "Get the top (most popular) media posts for a hashtag ID. Use search_hashtag first to get the hashtag ID.",
      inputSchema: {
    ...
    async (args) => {
      try {
        ...
        const data = await apiRequest(
          `/${args.hashtag_id}/top_media`,
          "GET",
          undefined,
          { user_id: INSTAGRAM_ACCOUNT_ID, fields },
        );
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to get hashtag top media: ${(error as Error).message}`);
      }
    },
  );
}
