...
export function registerMediaTools(server) {
    server.registerTool("list_media", {
        ...
        description: "List recent Instagram media posts with pagination. Returns posts in reverse chronological order.",
        inputSchema: {
            fields: z
                .string()
                .optional()
        ...
        try {
            ...
            const data = await apiRequest(`/${INSTAGRAM_ACCOUNT_ID}/media`, "GET", undefined, { fields, limit: args.limit || "25", after: args.after });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to list media: ${error.message}`);
        }
    });
    server.registerTool("list_stories", {
        title: "List Active Stories",
        description: "List currently active Instagram stories (only available for 24h after publishing).",
        inputSchema: {
            fields: z
                .string()
                .optional()
                .describe("Comma-separated fields. Default: id,media_type,media_url,timestamp,permalink"),
        },
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
        ...
        try {
            const fields = args.fields || "id,media_type,media_url,timestamp,permalink";
            const data = await apiRequest(`/${INSTAGRAM_ACCOUNT_ID}/stories`, "GET", undefined, { fields });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to list stories: ${error.message}`);
        }
    });
    server.registerTool("list_top_media", {
        title: "List Top Media by Engagement",
        description: "Fetch recent media and sort by engagement (likes + comments). Useful for finding best-performing posts.",
        inputSchema: {
            limit: z
                .string()
                .optional()
        ...
        try {
            ...
            const posts = await apiRequestAllPages(`/${INSTAGRAM_ACCOUNT_ID}/media`, { fields, limit: String(Math.min(limit, 100)) });
            const sorted = posts
                .slice(0, limit)
                .map((p) => ({
                ...p,
            ...
            return toolError(`Failed to list top media: ${error.message}`);
        }
    });
    server.registerTool("list_children", {
        title: "List Carousel Items",
        description: "List the individual items (children) of a carousel album post.",
        inputSchema: {
        ...
        try {
            ...
            const data = await apiRequest(`/${args.media_id}/children`, "GET", undefined, { fields });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to list children: ${error.message}`);
        }
    });
}
